...
function getFromStorage() {
    try {
        return JSON.parse(localStorage.taskFishCart);
    } catch (e) {
        return null;
    }
}

function setToStorage(id, quantity) {
    const setObject = {id: id, quantity: quantity};
    ...
    if (!storage) {
        storage = [];
        storage.push(setObject);
        localStorage.taskFishCart = JSON.stringify(storage);
        return;
    }
    
    const isID = storage.findIndex(item => item.id == id);

    if (isID) {
        console.log(storage[isID]['quantity']);
    }
...
products.forEach(product => {
    ...
    elProdAddButton.addEventListener('click', (e) => {
        ...
        if (ProdIsInCart) {
            ...
            setToStorage(id, Number(ProdIsInCart.quantity));
